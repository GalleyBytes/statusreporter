...
RUN USER=root cargo new statusreporter
WORKDIR /usr/src/statusreporter
...
# ------------------------------- -------------------------------
FROM scratch
COPY --from=builder /usr/local/cargo/bin/statusreporter .
CMD ["./statusreporter"]
LABEL org.opencontainers.image.source https://github.com/galleybytes/statusreporter
